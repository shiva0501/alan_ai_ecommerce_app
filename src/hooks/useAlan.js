...
export default function useAlan(){
    ...
    const removeItem = useCallback(({ detail: { name }}) => {
        ...
        if(entry == null) {
            alanInstance.playText(`I cannot find the ${name} item in the cart`);
        } else {
            removeFromCart(entry.item.id);
            alanInstance.playText(`Removed the ${name} item from the cart`);
        }
    ...
    const checkoutItems = useCallback(() => {
        if(isCartEmpty){
            ...
            alanInstance.playText('Purchasing the items in cart');
            checkout();
        }
    },[isCartEmpty, alanInstance, checkout]);
    ...
    useEffect(() => {
        ...
        window.addEventListener(COMMANDS.CHECKOUT_ITEMS, checkoutItems);
        ...
        return () => {
            ...
            window.removeEventListener(COMMANDS.CHECKOUT_ITEMS, checkoutItems);
        }
